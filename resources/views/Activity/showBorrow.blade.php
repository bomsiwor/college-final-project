        ...
        <div class="col-lg-12">
            <div class="card px-2" id="borrowDetailCard">
                <div class="card-body">
                    ...
                    <div class="container-fluid d-flex flex-wrap justify-content-between">
                        <div class="col-lg-3 col-6 ps-0">
                            <p class="mb-0 mt-3"><b>Kondisi barang</b></p>
                            <p>
                                Sebelum :
                                <span
                                    class="badge {{ __("core.$borrow->before_condition.class") }}">{{ __("core.$borrow->before_condition.text") }}</span>
                                <br />
                                Sesudah :
                                @if (!$borrow->after_condition && $borrow->verified_at && $borrow->status == 'accepted')
                                    <span class="badge badge-outline-dark mt-1">Belum kembali</span>
                                @elseif($borrow->after_condition)
                                    <span
                                        class="badge {{ __("core.$borrow->after_condition.class") }}">{{ __("core.$borrow->after_condition.text") }}</span>
                                @elseif($borrow->status == 'rejected')
                                    <span class="badge badge-outline-danger">Peminjaman ditolak</span>
                                @else
                                    <span class="text-muted">Belum diverifikasi</span>
                                @endif
                            </p>
                    ...
                    <div class="container-fluid mt-3 d-flex justify-content-center w-100">
                        <div class="table-responsive w-100">
                            <table class="table">
                                ...
                                <tbody>
                                    <tr class="text-right">
                                        <td class="text-left lh-lg">{{ $borrow->inventory->name }}
                                            <br>{{ $borrow->inventory->merk . '-' . $borrow->inventory->series }}
                                        </td>
                                        <td>{{ $borrow->inventory->inventory_number }}</td>
                    ...
                    @role('admin')
                        @empty($borrow->verified_at)
                            <div class="container-fluid w-100">
                                <form id="verification" action="{{ route('activity.borrow.verify') }}" method="POST">
                                    @csrf
